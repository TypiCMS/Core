        ...
        <div :class="{ 'item-list-content': !subList }">
            <div :class="{ header: !subList }" class="item-list-header">
                <div class="btn-toolbar item-list-toolbar header-toolbar">
                    <item-list-tree-selector
                        v-if="$can('update ' + table) || $can('delete ' + table)"
                        :all-checked="allChecked"
                        :filtered-models="flattenedModels"
                        :loading="loading"
                        :publishable="true"
                        @check-all="checkAll"
            ...
            <div class="content">
                <sl-vue-tree-next ref="slVueTree" v-model="models" @drop="drop" @toggle="toggle">
                    <template #title="{ node }">
                        ...
                        <div class="title">{{ translatable ? node.data.title_translated : node.data.title }} ••• {{ node.data.id }}</div>
...
import ItemListTreeActions from './ItemListTreeActions.vue';
import ItemListTreeSelector from './ItemListTreeSelector.vue';
...
const allChecked = computed(() => {
    return flattenedModels.value.length > 0 && flattenedModels.value.length === checkedItems.value.length;
});
...
function checkAll() {
    checkedItems.value = [...flattenedModels.value];
}
...
function checkPublished() {
    ...
    checkedItems.value = flattenedModels.value.filter((model) => model[statusVar] === 1);
}
...
function checkUnpublished() {
    ...
    checkedItems.value = flattenedModels.value.filter((model) => model[statusVar] === 0);
}
