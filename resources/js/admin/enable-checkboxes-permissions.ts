export default () => {
    ...
    labels.forEach((label, index) => {
        ...
        headingCheckbox.classList.add(
            'permissions-modules-item-title-checkbox',
            'form-check-input',
        );
        label.prepend(headingCheckbox);
    });

    const headingCheckboxes = document.querySelectorAll<HTMLInputElement>(
        '.permissions-modules-item-title-checkbox',
    );
    headingCheckboxes.forEach((headingCheckbox) => {
        ...
        headingCheckbox.addEventListener('click', (event) => {
            const element = event.target as HTMLInputElement,
                ...
                checkboxes =
                    parentItem?.querySelectorAll<HTMLInputElement>(
                        'input[type="checkbox"]:not(:disabled)',
                    ) ?? [];
            checkboxes.forEach(
                (headingCheckbox) =>
                    (headingCheckbox.checked = element.checked),
            );
        });
        ...
        const parentItem = headingCheckbox.closest('.permissions-modules-item'),
            numberOfCheckboxes =
                parentItem?.querySelectorAll<HTMLInputElement>(
                    'input[name="checked_permissions[]"]',
                ).length ?? 0,
            numberOfCheckedCheckboxes =
                parentItem?.querySelectorAll<HTMLInputElement>(
                    'input[name="checked_permissions[]"]:checked',
                ).length ?? 0;
