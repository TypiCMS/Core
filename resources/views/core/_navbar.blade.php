@if ($navbar)
    <nav class="typicms-navbar navbar navbar-expand bg-dark justify-content-between sticky-top" data-bs-theme="dark">
        <div class="container-fluid">
            ...
            @endif
            <a class="typicms-navbar-brand navbar-brand" href="{{ route('admin::dashboard') }}">{{ TypiCMS::title(config('typicms.navbar_locale')) }}</a>
            <ul class="navbar-nav ms-auto">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                        <svg class="bi bi-person-circle me-2" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16">
                    ...
                    <div class="dropdown-menu dropdown-user">
                        <div class="dropdown-user-wrapper">
                            ...
                            <div class="info">
                                ...
                                @can('update users')
                                    <div class="mb-3">
                                        <a href="{{ route('admin::edit-user', Auth::id()) }}">{{ __('Profile', [], config('typicms.navbar_locale')) }}</a>
                                    </div>
                                @endcan
                                <div class="mb-2">
                                    <form action="{{ route(TypiCMS::mainLocale().'::logout') }}" method="post">
                                        {{ csrf_field() }}
                                        <button class="btn btn-light btn-sm" type="submit">@lang('Logout', [], config('typicms.navbar_locale'))</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                @can('read settings')
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('admin::index-settings') }}">
                            ...
                            <span class="d-none d-lg-inline">{{ __('Settings', [], config('typicms.navbar_locale')) }}</span>
                        </a>
                    </li>
                @endcan
            </ul>
