<li
    class="{{ $name }}-nav-item {{ $name }}-nav-item-{{ $menulink->id }} {{ $menulink->class }}"
    id="menuitem_{{ $menulink->id }}"
    role="menuitem"
>
    <a
    ...
    >
        @if ($menulink->image !== null)
            <img
                class="{{ $name }}-nav-image"
                aria-hidden="true"
                src="{{ $menulink->present()->image(64, 64) }}"
                width="32"
                height="32"
                alt=""
            />
    ...
    @if ($menulink->items->count() > 0)
        <ul
            class="{{ $name }}-nav-dropdown dropdown-menu"
            aria-labelledby="menuitem_{{ $menulink->id }}_id"
            role="menu"
        >
