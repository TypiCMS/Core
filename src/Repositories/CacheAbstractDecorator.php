...
abstract class CacheAbstractDecorator implements RepositoryInterface
{
    ...
    public function byId($id, array $with = [])
    {
        ...
        $cacheKey = md5(config('app.locale').'id.'.serialize($with).$id.serialize(Input::all()));
    ...
    {
        // Build the cache key, unique per model slug
        $cacheKey = md5(config('app.locale').'getFirstBy'.$key.$value.serialize($with).$all.serialize(Input::all()));
    ...
    {
        $cacheKey = md5(config('app.locale').'byPage'.$page.$limit.serialize($with).$all.serialize(Input::except('page')));
    ...
    {
        $cacheKey = md5(config('app.locale').'all'.serialize($with).$all.serialize(Input::except('page')));
    ...
    {
        $cacheKey = md5(config('app.locale').'allNested'.serialize($with).$all.serialize(Input::except('page')));
    ...
    {
        $cacheKey = md5(config('app.locale').'allBy'.$key.$value.serialize($with).$all.serialize(Input::all()));
    ...
    {
        $cacheKey = md5(config('app.locale').'allNestedBy'.$key.$value.serialize($with).$all.serialize(Input::all()));
    ...
    {
        $cacheKey = md5(config('app.locale').'latest'.$number.serialize($with).serialize(Input::all()));
    ...
    {
        // Build the cache key, unique per model slug
        $cacheKey = md5(config('app.locale').'bySlug'.$slug.serialize($with).serialize(Input::all()));
