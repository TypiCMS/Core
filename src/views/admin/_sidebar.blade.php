...
<div class="col-xs-6 col-sm-3 col-md-2 sidebar sidebar-offcanvas" id="sidebar" role="navigation">
    ...
    <div class="sidebar-panel">
        @if ($section)
        <a class="sidebar-title @if(Config::get('typicms.user.menus_'.$section.'_collapsed'))collapsed @endif" href="#{{ $section }}" data-toggle="collapse">
            <div> @lang('global.menus.' . $section)</div>
        </a>
        @endif
        <div class="panel-collapse collapse @if(! Config::get('typicms.user.menus_'.$section.'_collapsed'))in @endif" id="{{ $section }}">
